...
log(
    'info',
    '[BUILD INFO] Release channel: stable'
        .concat(' Build Number: ', '{BUILD_NUMBER}')
        .concat(', Version Hash: ', '{VERSION_HASH}'),
    false,
);
...
(async () => {
    const r = await (
        await fetch(
            window.wavecaptcha.config.baseUrl.replaceAll(
                'api',
                'wv/1/t/js/api.js',
            ),
        )
...
async function loadCaptcha(sendMessge: any) {
    ...
    if (captcha?.action?.type === 'LOAD_TYPE_WORD_MODAL') {
        log(
            'info',
            'Asking user to type some text to get more behavioral data',
        );
